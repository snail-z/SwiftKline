...
- (void)updateLayout {
    ...
    _crosswireView.fadeHidden = YES;
    _crosswireView.separationRect = CGRectMake(self.layout.contentFrame.origin.x, self.layout.topChartFrame.size.height, self.layout.topChartFrame.size.width, self.layout.separatedGap);
    ...
    [self.style setValue:@(oneWidth) forKey:STRINGSEL(volumeShapeWidth)];
}
...
- (void)drawChart {
    ...
    if (self.style.chartType == TQTimeChartTypeFive) {
...
- (void)drawTimeGridLines {
    NSArray<NSString *> *leftTexts = [NSArray arrayWithPartition:self.style.timeGridSegments peakValue:self.timePeakValue];
    NSArray<NSString *> *rightTexts = [NSArray arrayWithPartition:self.style.timeGridSegments peakValue:self.changeRatioPeakValue];
...
- (void)drawVolumeGridLines {
    CGPeakValue peak = [self.dataArray peakValueBySel:@selector(tq_timeVolume)];
    NSArray<NSString *> *array = [NSArray arrayWithPartition:self.style.volumeGirdSegments peakValue:peak];
...
- (void)drawVolumeChart {
    CGPeakValue peak = [self.dataArray peakValueBySel:@selector(tq_timeVolume)]; // should optimized
    CGaxisYConverBlock pyCallback = CGaxisYConverMaker(peak, self.layout.bottomChartFrame, self.style.gridLineWidth);
...
/** 绘制分时图 */
- (void)drawTimeChart {
    [self drawTimeDateLines]; // 绘制单日分时日期线
    
    CGaxisYConverBlock pYCallback = CGaxisYConverMaker(self.timePeakValue, self.layout.topChartFrame, self.style.gridLineWidth);
...
- (void)drawTimeDateLines {
    if (!self.style.dateTexts.count) return;
    ...
    if (self.style.dateTexts.count == 1) {
        CGFloat centerX = CGRectGetMidX(self.layout.contentFrame);
        ...
        makeRender(self.style.dateTexts.firstObject, kCGOffsetRatioCenter, CGRectGetMidX(self.layout.contentFrame));
        _dateGridLayer.path = path.CGPath;
        _dateLineTextLayer.renders = renders;
        return;
    }
    NSRange range = NSMakeRange(0, 0);
    if (self.style.dateTexts.count > 1) range = NSMakeRange(1, self.style.dateTexts.count - 2);
    NSInteger section = self.style.maxDataCount / MAX(1, self.style.dateTexts.count - 1);
    
    makeRender(self.style.dateTexts.firstObject, kCGOffsetRatioCenterLeft, CGRectGetMinX(self.layout.contentFrame));
    [self.style.dateTexts enumerateObjsAtRange:range ceaselessBlock:^(NSString * _Nonnull text, NSUInteger idx) {
        CGFloat centerX = [self getCenterXWithIndex:(idx * section)];
        [path addVerticalLine:CGPointMake(centerX, CGRectGetMinY(self.layout.topChartFrame)) len:self.layout.topChartFrame.size.height];
        [path addVerticalLine:CGPointMake(centerX, CGRectGetMinY(self.layout.bottomChartFrame)) len:self.layout.bottomChartFrame.size.height];
        makeRender(text, kCGOffsetRatioCenter, centerX);
    }];
    makeRender(self.style.dateTexts.lastObject, kCGOffsetRatioCenterRight, CGRectGetMaxX(self.layout.contentFrame));
    _dateGridLayer.path = path.CGPath;
    _dateLineTextLayer.renders = renders;
}

#pragma mark - 'Five' Time-sharing chart

/** 绘制五日分时图 */
- (void)drawFiveTimeChart {
    NSMutableIndexSet *dateIndexSet = [NSMutableIndexSet indexSet];
    NSInteger dayCount = self.style.maxDataCount / self.style.chartType;
    NSInteger dataCount = self.dataArray.count;
    for (NSInteger i = 0; i < self.style.chartType; i++) {
        NSInteger index = i * dayCount;
        if (index < dataCount) [dateIndexSet addIndex:index];
    }
    [self drawFiveDateLinesWithIndexSet:dateIndexSet]; // 绘制五日分时日期线

    CGFloat(^pyCallback)(CGFloat) = CGaxisYConverMaker(self.timePeakValue, self.layout.topChartFrame, 1);
    CGFloat halfTimeLineWith = half(self.style.timeLineWith);
    id<TQTimeChartProtocol>firstObj = self.dataArray.firstObject;
    CGFloat firstPointY = pyCallback(self.coordsConfig.tq_originPrice) + halfTimeLineWith;
    ...
    CGFloat lastCenterX = [self getCenterXWithIndex:self.dataArray.lastIdx];
    [fillPath addLineToPoint:CGPointMake(lastCenterX, CGRectGetMaxY(self.layout.topChartFrame))];
    [fillPath addLineToPoint:CGPointMake(self.layout.topChartFrame.origin.x, CGRectGetMaxY(self.layout.topChartFrame))];
    [fillPath closePath];
    _timeLineFillLayer.path = fillPath.CGPath;
...
- (void)drawFiveDateLinesWithIndexSet:(NSIndexSet *)indexSet {
    CGFloat positionCenterY; CGFloat baseOffsetVertical;
    [self makeDateRenderer:&positionCenterY baseOffsetVertical:&baseOffsetVertical];
    CGFloat gap = self.layout.topChartFrame.size.width / indexSet.count;
...
#pragma mark - GestureInitialization / Gestures management
...
- (void)longPress:(UILongPressGestureRecognizer *)g {
    switch (g.state) {
        case UIGestureRecognizerStateBegan: {
            if ([self.delegate respondsToSelector:@selector(stockTimeChartWillLongPress:)]) {
                [self.delegate stockTimeChartWillLongPress:self];
            }
...
- (void)updateCrosswireLayerWithPoint:(CGPoint)p {
    ...
    if (CGRectContainsPoint(self.layout.topChartFrame, p) && isNeedsDisplay) {
        CGFloat value = CGaxisYToValueBlock(self.timePeakValue, self.layout.topChartFrame, p.y);
        CGFloat subValue = CGaxisYToValueBlock(self.changeRatioPeakValue, self.layout.topChartFrame, p.y);
        _crosswireView.yaixsText = NS_StringFromRoundFloatKeep2(value);
        if (CG_FloatIs2fZero(subValue)) subValue = 0.00;
        _crosswireView.yaixsSubText = NS_StringFromRoundFloatKeep2(subValue);
    }
    if (CGRectContainsPoint(self.layout.bottomChartFrame, p) && isNeedsDisplay) {
        CGPeakValue peak = [self.dataArray peakValueBySel:@selector(tq_timeVolume)];
        CGFloat value = CGaxisYToValueBlock(peak, self.layout.bottomChartFrame, p.y);
        _crosswireView.yaixsText = NS_StringFromRoundFloatKeep2(value);
        _crosswireView.yaixsSubText = nil;
    }
    p.x -= self.layout.contentFrame.origin.x; p.y -= self.layout.contentFrame.origin.y;
    NSInteger index = CGaxisXToIndexBlock(p.x, self.style.volumeShapeWidth, self.style.volumeShapeGap, self.dataArray.count);
    if (isNeedsDisplay) {
        id<TQTimeChartProtocol>obj = self.dataArray[index];
        NSString *formatter = TQChartTimeTouchDateFormatter;
        if (self.style.chartType == TQTimeChartTypeFive) formatter = TQChartFiveTimeTouchDateFormatter;
        NSString *mapText = [TQChartDateManager.sharedManager stringFromDate:obj.tq_timeDate dateFormat:formatter];
