//
//  TQStockIndicatorSegmentedBar.m
//  TQChartKit
//
...
@interface TQStockIndicatorSegmentedBar () <UITableViewDelegate, UITableViewDataSource>
...
@implementation TQStockIndicatorSegmentedBar
...
- (void)defaultInitialization {
    ...
    _selectedTextColor = [UIColor zh_colorWithHexString:@"#4169E1"];
    _selectedTextFont = [UIFont boldSystemFontOfSize:12];
    _unselectedTextFont = [UIFont systemFontOfSize:12];
...
- (void)reloadHeaderView {
    ...
    [_headerTitles enumerateObjectsUsingBlock:^(NSString * _Nonnull text, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        label.text = text;
        BOOL isSelected = (label.tag == self->_headerView.tag);
        label.textColor = isSelected ? self.selectedTextColor : self.unselectedTextColor;
        label.font = isSelected ? self.selectedTextFont : self.unselectedTextFont;
...
- (void)headerLabelClicked:(UITapGestureRecognizer *)g {
    if ([self.delegate respondsToSelector:@selector(stockIndicatorSegmentedBar:didClickHeaderItemAtIndex:)]) {
        [self.delegate stockIndicatorSegmentedBar:self didClickHeaderItemAtIndex:g.view.tag];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!cell) {
        ...
        cell.titleLabel.font = self.unselectedTextFont;
    }
    cell.titleLabel.text = _titles[indexPath.row];
    BOOL isSelected = (indexPath.row == tableView.tag);
    cell.titleLabel.textColor = isSelected ? self.selectedTextColor : self.unselectedTextColor;
    cell.titleLabel.font = isSelected ? self.selectedTextFont : self.unselectedTextFont;
    return cell;
}

- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if ([self.delegate respondsToSelector:@selector(stockIndicatorSegmentedBar:didClickItemAtIndex:)]) {
        [self.delegate stockIndicatorSegmentedBar:self didClickItemAtIndex:indexPath.row];
    }
}
