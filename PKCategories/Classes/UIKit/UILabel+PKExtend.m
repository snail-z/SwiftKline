...
- (CGFloat)pk_estimatedWidth {
    return [self sizeThatFits:CGSizeMake(CGFLOAT_MAX, self.bounds.size.height)].width;
}

- (CGFloat)pk_estimatedHeight {
    return [self sizeThatFits:CGSizeMake(self.bounds.size.width, CGFLOAT_MAX)].height;
}

@end


@implementation UILabel (PKTextEdge)
