...
typedef void(^KVOBlock)(NSObject *sender, NSDictionary<NSKeyValueChangeKey,id> *change);

@interface _PKObserverWrapper : NSObject

@property (nonatomic, strong) NSObject *target;
@property (nonatomic, assign) NSKeyValueObservingOptions options;
@property (nonatomic, assign) NSMutableDictionary<NSString *, NSArray<KVOBlock> *> *pathBlocks;

@end

@implementation _PKObserverWrapper

- (NSMutableDictionary<NSString *,NSArray<KVOBlock> *> *)pathBlocks {
    if (!_pathBlocks) {
        _pathBlocks = [NSMutableDictionary dictionary];
    }
    return _pathBlocks;
}

- (instancetype)initWithTarget:(NSObject *)target keyPath:(NSString *)keyPath block:(KVOBlock)block {
    return [self initWithTarget:target options:NSKeyValueObservingOptionNew keyPath:keyPath block:block];
}

- (instancetype)initWithTarget:(NSObject *)target
                       options:(NSKeyValueObservingOptions)options
                       keyPath:(NSString *)keyPath block:(KVOBlock)block {
    if (self = [super init]) {
        _target = target;
        _options = options;
     
        [self.pathBlocks setValue:@[[block copy]] forKeyPath:keyPath];
        
        for (NSString *pathKey in self.pathBlocks.allKeys) {
            [_target addObserver:self forKeyPath:pathKey options:_options context:nil];
        }
    }
    return self;
}

- (instancetype)initWithWrapper:(_PKObserverWrapper *)wrapper keyPath:(NSString *)keyPath block:(KVOBlock)block {
    if (self = [super init]) {
        _target = wrapper.target;
        _options = wrapper.options;
        [self.pathBlocks removeAllObjects];
        
        for (NSString *pathKey in wrapper.pathBlocks) {
            id element = [wrapper.pathBlocks mutableArrayValueForKey:pathKey];
            [self.pathBlocks setValue:element forKey:pathKey];
        }
    
        NSMutableArray *blocks = [[self.pathBlocks mutableArrayValueForKey:keyPath] mutableCopy];
        if (blocks.count > 0) {
            [blocks addObject:block];
            [self.pathBlocks setValue:blocks forKey:keyPath];
        } else {
            [self.pathBlocks setValue:@[[block copy]] forKey:keyPath];
        }
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    if (object && keyPath) {
        NSArray<KVOBlock> *blocks = [self.pathBlocks mutableArrayValueForKey:keyPath];
        for (KVOBlock block in blocks) {
            block(object, change);
        }
    }
}
...
- (void)dealloc {
    for (NSString *pathKey in self.pathBlocks.allKeys) {
        [_target removeObserver:self forKeyPath:pathKey];
    }
}
