...
open class UGradientLayer: UBaseLayer {
    ...
    /// 渐变色数组
    public var gradientClolors: [NSColor?]? {
        didSet {
            updatePath(gradientPath)
        }
    }
    ...
    override func initialization() {
        ...
        maskLayer.strokeColor = NSColor.clear.cgColor
    ...
    private func updatePath(_ path: CGPath?) {
        ...
        if colors.count > 1 {
            ...
            gradientLayer.colors = colors.map({ $0?.cgColor ?? NSColor.black.cgColor })
            maskLayer.path = path
            guard !isDisablePathActions else { return }
            maskLayer.pathActions(duration: 1.25)
        } else {
            backgroundLayer.mask = maskLayer
