...
internal class UTimeCalculate {
    ...
    private func calculatePricePeakValue() -> UPeakValue {
        if
            let max = target.referenceSystem?._maxPrice,
            let min = target.referenceSystem?._minPrice {
    ...
    private func calculateChangeRatePeakValue() -> UPeakValue {
        if
            let max = target.referenceSystem?._maxChangeRate,
            let min = target.referenceSystem?._minChangeRate {
            return UPeakValue(max: max, min: min)
        } else {
    ...
    private func calculateVolumePeakValue() -> UPeakValue {
        if
            let max = target.referenceSystem?._maxVolume,
