...
//        _innerSpacing = sumSpacing / (self.buttonsView.subviews.count - 1);
    ...
    CGFloat contentWidth = CGRectGetMaxX(self.buttonsView.subviews.lastObject.frame) + self.paddingInset;
    self.scrollView.contentSize = CGSizeMake(contentWidth, self.bounds.size.height);
    self.buttonsView.frame = (CGRect){.size.width = contentWidth, .size.height = self.bounds.size.height};
}

- (CGFloat)elementWidthForIndex:(NSInteger)index {
    if (_equalWidthEnabled) {
        return self.bounds.size.width / self.buttonsView.subviews.count;
    } else {
        UILabel *label = self.buttonsView.subviews[index];
        return [label sizeThatFits:CGSizeMake(CGFLOAT_MAX, self.bounds.size.height)].width;
    }
}

- (CGFloat)elementTextWidthForIndex:(NSInteger)index {
    UILabel *label = self.buttonsView.subviews[index];
    return [label sizeThatFits:CGSizeMake(CGFLOAT_MAX, self.bounds.size.height)].width;
}
...
- (CGRect)elementFrameForIndex:(NSInteger)index {
    ...
    if (index > 0) {
        originX = CGRectGetMaxX(self.buttonsView.subviews[index - 1].frame) + self.innerSpacing;
    }
    return (CGRect){.origin.x = originX, .size = CGSizeMake(elementWidth, self.bounds.size.height)};
}

- (void)moveIndicatorView {
    CGFloat textWidth = [self elementTextWidthForIndex:self.index];
...
- (void)contentOffsetUpdates {
    UILabel *selectedLabel = self.buttonsView.subviews[self.index];
...
- (void)removeAllSubviews {
    while (self.buttonsView.subviews.count) {
        [self.buttonsView.subviews.lastObject removeFromSuperview];
    }
}
...
- (void)updateSegmentedTitles:(NSArray<NSString *> *)titles {
    if (titles.count) {
        ...
        for (int index = 0; index < titles.count; index++) {
            ...
            [self.buttonsView addSubview:titleLabel];
...
- (NSInteger)nearestIndexToPoint:(CGPoint)point {
    ...
    NSInteger lastIndex = self.buttonsView.subviews.count - 1;
    for (UIView *aView in self.buttonsView.subviews) {
...
- (void)textLabelsColorUpdates {
    for (UILabel *label in self.buttonsView.subviews) {
...
- (void)setPaddingInset:(CGFloat)paddingInset {
    if (self.equalWidthEnabled) return;
...
- (void)setEqualWidthEnabled:(BOOL)equalWidthEnabled {
    _equalWidthEnabled = equalWidthEnabled;
    _innerSpacing = 0;
    _paddingInset = 0;
    [self setNeedsLayout];
}
...
- (void)updateWithProgress:(CGFloat)progress {
    ...
    UILabel *leftLabel = self.buttonsView.subviews[leftIndex];
    UILabel *rightLabel = self.buttonsView.subviews[rightIndex];
